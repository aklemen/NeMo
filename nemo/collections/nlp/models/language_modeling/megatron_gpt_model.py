...
class MegatronGPTModel(MegatronBaseModel, TextGeneration):
    ...
    def fwd_bwd_step(self, dataloader_iter, forward_only, first_val_step=None):
        ...
        logging.info(f'losses_reduced_per_micro_batch: {losses_reduced_per_micro_batch}')
        ...
        if losses_reduced_per_micro_batch:
            if (not forward_only) or self.validation_drop_last:
                ...
                logging.info(f'loss_tensors_list: {loss_tensors_list}')
                loss_tensor = torch.concat(loss_tensors_list)
                logging.info(f'loss_tensor: {loss_tensor}')
                ...
                logging.info(f'loss_sum_tensors_list: {loss_sum_tensors_list}')
                loss_sum = (
                    torch.vstack(loss_sum_tensors_list).sum(axis=0)
                    if len(loss_sum_tensors_list) > 0
                    else torch.tensor([0.0, 0.0]).cuda()
                )
                logging.info(f'loss_sum: {loss_sum}')
                return loss_sum
        else:
